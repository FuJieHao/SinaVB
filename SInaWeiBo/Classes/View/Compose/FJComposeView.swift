...
//    fileprivate lazy var bgImageView: UIImageView = UIImageView(image: self.getScreenShot())
    ...
    //  获取 plist 数组模型数据
    fileprivate lazy var composeMenuArray: [FJComposeMenu] = self.loadComposeData()
...
//        startPopAnimation(isUp: true)
    ...
    private func addChildButton() {
        ...
        for i in 0..<composeMenuArray.count {
            ...
            let button = FJComposeMenuButton()
            //  设置 tag
            button.tag = i
            //  添加点击事件
            ...
            button.y = CGFloat(rowIndex) * buttonMargin + CGFloat(rowIndex) * buttonHeight + ScreenHeight
    ...
    //  加载 plist 数据
    fileprivate func loadComposeData() -> [FJComposeMenu] {
        //  plist 文件路径
        ...
        //  使用 YYModel 完成字典转模型
        let modelArray = NSArray.yy_modelArray(with: FJComposeMenu.self, json: dicArray) as! [FJComposeMenu]
